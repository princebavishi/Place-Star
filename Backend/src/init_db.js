...
if (usePostgres) {
    (async () => {
        ...
        try {
            ...
            // Create PostgreSQL compatibility functions matching SQLite/MySQL custom functions
            await client.query(`
            ...
            await client.query(`
                ...
                $$ LANGUAGE sql;
            `);
            console.log("PostgreSQL compatibility functions created.");
            ...
            // Seed default users from sql dump file if exists
            if (sqlFile && fs.existsSync(sqlFile)) {
                const content = fs.readFileSync(sqlFile, 'utf8');
                const insertRegex = /INSERT INTO \`users\` [\s\S]*?;/g;
                const match = content.match(insertRegex);
                if (match) {
                    let insertSql = match[0]
                        .replace(/`users`/g, 'users')
                        .replace(/`\w+`/g, m => m.slice(1, -1)) // Remove backticks from column names
                        .replace(/'Not Verified'/g, "'Verified'");
                    
                    insertSql = insertSql.replace(/INSERT INTO/i, 'INSERT INTO');
                    insertSql += ' ON CONFLICT (Username) DO NOTHING';
            ...
            // Seed custom demo credentials
            console.log("Seeding custom demo credentials...");
            await client.query(`
                INSERT INTO users (FirstName, LastName, Email, Username, Password, Role, sem, class, batch, Status, AccountStatus) VALUES
            ...
            await client.query(`
                INSERT INTO exams (ExamID, Title, Description, Subject, Number_of_Questions, Exam_Total_Marks, Status, ExamDate, StartTime, EndTime, CreatorID, sem, className, batch)
            ...
            await client.query(`
                INSERT INTO questions (QuestionID, ExamID, QuestionText, Mark, QuestionType, Correct_Option)
            ...
            await client.query(`
                INSERT INTO questionoptions (OptionID, QuestionID, OptionA, OptionB, OptionC, OptionD)
